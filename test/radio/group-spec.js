...
describe('Radio.Group', () => {
    ...
    describe('test focus', () => {
        beforeEach(() => {
            const lastContainer = document.querySelectorAll('.container');
            lastContainer.forEach(container => {
        ...
        it('should focus on first radio', () => {
            class App extends BaseComponent {
                render() {
                    return (
                        ...
                        >
                            <Radio dataIndex={1}>1</Radio>
                            <Radio dataIndex={2}>2</Radio>
                        </Radio.Group>
                    );
                }
            }
            ...
            assert(document.activeElement.getAttribute('dataindex') === '1');
        });

        it('should focus on selected radio', () => {
            class App extends BaseComponent {
                render() {
                    return (
                        <Radio.Group defaultValue={2} ref={el => this.setupRef(el)}>
                            <Radio dataIndex={1} value={1}>
                                1
                            </Radio>
                            <Radio dataIndex={2} value={2}>
                                2
                            </Radio>
                        </Radio.Group>
                    );
            ...
            insertDom(App);
            assert(document.activeElement.tagName.toUpperCase() === 'INPUT');
            assert(document.activeElement.getAttribute('dataindex') === '2');
        });

        it('should focus on radio in datasource mode', () => {
            class App extends BaseComponent {
                render() {
                    return <Radio.Group ref={el => this.setupRef(el)} dataSource={list} />;
                }
            }
        ...
        it('should NOT be focused if there is no radio in group', () => {
            class App extends BaseComponent {
                render() {
                    return <Radio.Group ref={el => this.setupRef(el)} />;
                }
            }
            insertDom(App);
            assert(document.activeElement.tagName.toUpperCase() === 'BODY');
        });
    });
